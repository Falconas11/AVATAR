...
class KnowledgeEngine:
    ...
    def build_factor_activations(self, profile: dict[str, Any]) -> dict[str, float]:
        ...
        # Convert the requested age progression into a graph activation.
        # Adult morphological aging is normalized over ages 20-80 so that a
        # 20 -> 65 edit yields 0.75, while edits at or below age 20 yield 0.
        if "intrinsic_aging" in self.nodes:
            current_age = float(profile.get("age", 20))
            target_age = float(profile.get("target_age", current_age))
            if target_age > current_age:
                age_activation = (target_age - 20.0) / 60.0
                activations["intrinsic_aging"] = self._clamp(age_activation)
        return activations

    def _edge_is_eligible(self, edge: Edge, profile: dict[str, Any]) -> bool:
        # Edge age constraints describe the rendered target state. Fall back
        # to current age for profiles that do not specify a target age.
        age = profile.get("target_age", profile.get("age"))
        if age is not None:
            age = int(age)
    ...
    def infer(self, profile: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "propagation": "topological_multi_hop_weighted_mean_with_target_age_activation",
